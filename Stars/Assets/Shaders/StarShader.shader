...
Shader "Unlit/StarShader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                half2 uv_centered = (2 * i.uv) - 1;
                half distance_from_centre = length(uv_centered);
                half inverse_dist = saturate((0.2 / distance_from_centre) - 0.2);
                fixed4 col = fixed4(_Color.rgb, inverse_dist);
                UNITY_APPLY_FOG(i.fogCoord, col);
                return col;
            }
            ENDCG
